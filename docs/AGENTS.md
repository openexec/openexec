...
- `ui/` Vite + React (TypeScript). Tests under `ui/src/**/__tests__`; Playwright e2e in `ui/e2e/`. Built assets in `ui/dist/`.
- `docs/`, `scripts/`, `.openexec/` (local data; audit DB at `.openexec/data/audit.db`).

## Build, Test, and Development Commands
- Build: `go build -o bin/openexec ./cmd/openexec` (all OS)
- Backend start: `./bin/openexec start` (serves `:8080`; `--daemon` to background)
- Go tests: `go test ./...`
