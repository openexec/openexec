...
- **Append-only, tamper-evident, atomic audit.** `decision_events` and `evidence` are append-only (DB triggers reject UPDATE/DELETE). Decision events form a SHA-256 **hash chain** (`hash = SHA256(prev_hash ‖ canonical(event))`); `VerifyAuditChain` detects any alteration/deletion/reorder; ordering is by rowid. A **change's** status change and its decision event are written in **one transaction** (`TransitionChange`) — either both land or neither — for approve, mark-done, record-PR, ready-for-test, reject/defer, revise, flat/deep triage→plan_ready, quickplan→plan_ready, claim→implementing, and AI-review→changes_requested. (Two intentional exceptions: *release-level* events are recorded separately, lower stakes; and a claim's soft **lease** is taken first as its own atomic conditional UPDATE — a race loser never advances status — so "claim→implementing" atomicity means the *status change + its event*, not the lease acquisition.) Merge records `merge_authorized` before the `gh` call and `merged`/`merge_failed` after, so an external side effect is never silent.
...
**Third review round** (two follow-ups, both closed): the flat (non-deep) `svc.Triage`/`ai.Triage` path now uses `TransitionChange` too, so *all* triage variants are atomic; and the atomicity bullet above now states explicitly that a claim's lease acquisition is a separate prior step (not part of the status+event transaction).

Remaining open (reviewer's own recommendations, lower priority): the deterministic planner-verification linter (`planner.LintVerificationScript`) is advisory, not a hard reject; per-story execute scoping, release-level atomicity, and a full cryptographic operator-identity model are still future work.
