...
func TestAPIProviderAdvertisesOnlyConfiguredModel(t *testing.T) {
	provider, err := newConfiguredAPIProvider(writeProjectConfig(t, "qwen-gpu0"), "qwen-gpu0", readOnly)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAPIProviderSelectionIgnoresActiveProvider(t *testing.T) {
	...
	provider, err := newConfiguredAPIProvider(directory, "qwen-coder-gpu1", readOnly)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAPIProviderUnknownNameListsConfigured(t *testing.T) {
	_, err := newConfiguredAPIProvider(writeProjectConfig(t, "qwen-gpu0"), "qwen-gpu7", readOnly)
	if err == nil {
...
func TestAPIProviderRejectsUnresolvedKeyReference(t *testing.T) {
	_, err := newConfiguredAPIProvider(writeProjectConfig(t, "qwen-gpu0"), "needs-env", readOnly)
...
func TestAPIProviderRequiresName(t *testing.T) {
	if _, err := newConfiguredAPIProvider(writeProjectConfig(t, ""), "", readOnly); err == nil {
...
func TestAPIProviderLiveOllama(t *testing.T) {
	...
	directory := writeProjectConfig(t, "qwen-gpu0")
	provider, err := newConfiguredAPIProvider(directory, "qwen-gpu0", readOnly)
	if err != nil {
		t.Fatal(err)
	}
