...
type Server struct {
	...
	// Governance MCP tools now live in the governance module (internal/governance/
	// mcpgov) and are registered as a provider by the composition root, so core
	// does not import governance. See internal/mcptool.
...
func (s *Server) handleToolsList(req Request) {
	...
	tools := []interface{}{
		...
		MemoryReadToolDef(),
		SkillProposeToolDef(),
		// Release-governance tools are now contributed by the governance provider
		// (see providerToolDefs, appended below) — no longer built into core.
	}
...
func (s *Server) handleToolsCall(req Request) {
	...
	case "skill_propose":
		...
		telemetry.RecordToolSuccess(span, "")
	// Release-governance executor-handoff tools now come from the governance
	// provider (dispatched above via dispatchProvider), not this switch.
