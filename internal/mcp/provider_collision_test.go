...
// TestGovernanceProviderHasNoReservedCollision guards the real module: none of
// governance's tool names may intersect the reserved core set, so it always
// registers cleanly.
func TestGovernanceProviderHasNoReservedCollision(t *testing.T) {
	reserved := reservedCoreToolNames()
	for name := range mcpgov.New().Tools() {
		if reserved[name] {
			t.Errorf("governance tool %q collides with a reserved core tool name", name)
		}
	}
}
