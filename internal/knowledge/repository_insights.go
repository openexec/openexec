...
func (s *Store) deadCodeCandidates(ctx context.Context, generationID string) ([]InsightSymbol, bool, error) {
	...
	if err != nil {
		return nil, false, fmt.Errorf("dead-code candidates: %w", err)
	}
	defer rows.Close()
	...
	truncated := len(out) > deadCodeCandidateLimit
	if truncated {
...
func (s *Store) hotspots(ctx context.Context, generationID string) ([]InsightSymbol, error) {
	rows, err := s.db.QueryContext(ctx, `
		...
		GROUP BY n.id ORDER BY inbound DESC, rs.display_name LIMIT ?`, generationID, hotspotLimit)
	if err != nil {
...
func (s *Store) moduleSketch(ctx context.Context, generationID string) ([]ModuleFlow, error) {
	rows, err := s.db.QueryContext(ctx, `
	...
	if err != nil {
		return nil, fmt.Errorf("module sketch: %w", err)
	}
	defer rows.Close()
...
func (s *Store) attachRepositoryInsights(ctx context.Context, generationID string, projection *RepositoryContextProjection) error {
	...
	dead, truncated, err := s.deadCodeCandidates(ctx, generationID)
	if err != nil {
		return err
	}
	...
	if projection.Hotspots, err = s.hotspots(ctx, generationID); err != nil {
		return err
	}
	if projection.ModuleSketch, err = s.moduleSketch(ctx, generationID); err != nil {
		return err
	}
	return nil
}
