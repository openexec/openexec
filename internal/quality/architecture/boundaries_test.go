// Package architecture enforces the OpenExec core/module dependency rule. It
// reads package_boundaries.yaml, computes the actual import graph via `go list`,
// and fails on any boundary violation not recorded in the manifest's baseline
// (a ratchet: no NEW coupling, while the recorded debt is driven down over time).
