...
**Approval & tiering (P3):** apply-class invocations request approval in-handler (dry-runs `check`/`test` don't) through `internal/approval.PersistentGate` (`.openexec/approvals.db`, cross-process polling, default wait 30m / `OPENEXEC_APPROVAL_WAIT`), failing closed without a gate. `risk_profile: low` environments run applies autonomously; destructive terraform changes always need a human. Sign-off: `openexec approve list|yes|no <id> --local`, or `approval_list`/`approval_decide` MCP tools in an `OPENEXEC_OPERATOR_SESSION=1` server only (never reuse backlog tools for approval — their read-only-mode exception would become an authorization bypass; agent sessions must never self-approve).
